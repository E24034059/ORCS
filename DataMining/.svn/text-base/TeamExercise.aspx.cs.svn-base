...
public partial class DataMining_TeamExercise : ORCS.ORCS_Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (Request.QueryString["ddlTeamID"] != null)
            {
                string strStudent = Request.QueryString["ddlTeamID"].ToString();
                //檢查下拉式選單是否該項目，若有則選定其項目
    ...
    protected void LoadTeam()
    {
        if (ddlClass.Items.Count > 0)
        {
            ...
            ddlStudent.Items.Clear();
            //取出課程下所有小組ID
            DataTable dtTempGroup = clsEditGroup.ORCS_TempGroup_SELECT_by_iClassGroupID(ddlClass.SelectedItem.Value);
            if (dtTempGroup.Rows.Count > 0)
            {
                foreach (DataRow drTempGroup in dtTempGroup.Rows)
                {
                    //加入DropDownList
                    ddlStudent.Items.Add(new ListItem(drTempGroup["cTempGroupName"].ToString(), drTempGroup["iTempGroupID"].ToString()));
                }
            }
        }
    }
                    ...
                    if (dtExerciseQuesHis.Rows.Count > 0)
                    {
                        foreach (DataRow drExerciseQuesHis in dtExerciseQuesHis.Rows)
                        {
                            ...
                            if (dtExerClassHis.Rows.Count > 0)
                            {
                                ...
                                string strNoAns = "未作答人數: " + iNoAns + "組";
                                string strFinAns = "已作答人數: " + iFinAns + "組";
                                //呼叫javacript畫圖餅圖函數
